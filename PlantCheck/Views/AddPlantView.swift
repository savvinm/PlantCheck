...
struct AddPlantView: View {
        ...
        VStack(alignment: .leading) {
            ...
            VStack {
                TextField("Name (optional)", text: $vm.name)
                    ...
                    .overlay(alignment: .trailing) {
                        if vm.name != ""{
                            Button(action: { vm.name = "" }, label: { clearButtonIcon })
                        }
                    }
            ...
            VStack {
                TextField("Location (optional)", text: $vm.location)
                    ...
                    .overlay(alignment: .trailing) {
                        if vm.location != ""{
                            Button(action: { vm.location = "" }, label: { clearButtonIcon })
                        }
                    }
    ...
    private var genusInput: some View {
        HStack {
            Image(systemName: "magnifyingglass")
                .foregroundColor(.secondary)
            TextField("Search", text: $vm.genus, onEditingChanged: { (isBegin) in
            ...
            .overlay(alignment: .trailing) {
                if vm.genus != ""{
                    Button(action: { vm.genus = "" }, label: { clearButtonIcon })
                }
            }
        }
        .padding(.leading)
